...
async def _process_audio_in_chunks(audio_data: bytes, stt_tool) -> str:
    """
    Process large audio files by pre-chunking using ffmpeg
    ...
    """
    try:
        print(f"🔪 Pre-chunking large audio file ({len(audio_data)/1024/1024:.2f}MB) using ffmpeg")
        
        # Check if ffmpeg is available
        if not shutil.which('ffmpeg'):
            print("⚠️ ffmpeg not available. Attempting direct submission with chunking_strategy...")
            # Let Azure handle it with chunking_strategy
        ...
        try:
            ...
            if probe_result.returncode != 0:
                print("⚠️ Could not determine duration. Using direct submission with chunking_strategy...")
                result = await stt_tool.transcribe_audio(audio_data, _detect_audio_format(audio_data))
                return result.get("transcription", f"Processing failed") if result.get("success") else f"Failed: {result.get('error')}"
            ...
            if not duration or duration < 10:
                print("⚠️ Audio too short or invalid. Processing directly...")
                result = await stt_tool.transcribe_audio(audio_data, _detect_audio_format(audio_data))
                return result.get("transcription", "No transcription") if result.get("success") else f"Failed: {result.get('error')}"
            
            print(f"📊 Audio duration: {duration:.1f} seconds")
            
            # Calculate chunk duration (target ~4MB per chunk)
            estimated_bitrate = (len(audio_data) * 8) / duration
            target_chunk_duration = (4 * 1024 * 1024 * 8) / estimated_bitrate if estimated_bitrate > 0 else 300
            
            # Ensure 5-10 minute chunks
            chunk_duration = max(300, min(600, target_chunk_duration))
            num_chunks = int((duration / chunk_duration) + 1)
            
            print(f"🔢 Creating {num_chunks} chunks of ~{chunk_duration:.0f}s ({chunk_duration/60:.1f} min) each")
            ...
            for i in range(num_chunks):
                ...
                print(f"🎬 Chunk {i+1}/{num_chunks}: {start_time:.1f}s to {start_time + current_duration:.1f}s")
                
                chunk_path = f"{input_path}_chunk_{i}.wav"
                
                # Extract chunk as PCM16 mono 16kHz
                chunk_cmd = [
                    'ffmpeg', '-y', '-hide_banner', '-loglevel', 'error',
                    '-i', input_path,
                    '-ss', str(start_time),
                ...
                if chunk_result.returncode != 0:
                    print(f"⚠️ Chunk {i+1} extraction failed")
                    continue
                ...
                try:
                    ...
                    print(f"📤 Transcribing chunk {i+1} ({len(chunk_data)/1024:.0f}KB)...")
                    
                    # Use retry function for robustness
                    chunk_text = await stt_tool.transcribe_with_retries(chunk_data, max_retries=2)
                    
                    if chunk_text and "failed" not in chunk_text.lower():
                        transcriptions.append(chunk_text.strip())
                        print(f"✅ Chunk {i+1}: {len(chunk_text)} characters")
                    else:
                        print(f"⚠️ Chunk {i+1} failed: {chunk_text}")
                
                except Exception as e:
                    print(f"⚠️ Error with chunk {i+1}: {e}")
            ...
            if transcriptions:
                ...
                print(f"🎉 Combined {len(transcriptions)} chunks: {len(combined)} total characters")
                return combined
            else:
                return "No chunks were successfully transcribed."
        ...
        print("🔄 Attempting direct submission as fallback...")
        try:
            result = await stt_tool.transcribe_audio(audio_data, _detect_audio_format(audio_data))
            return result.get("transcription", f"Error: {e}") if result.get("success") else f"Failed: {result.get('error')}"
        except Exception as fallback_error:
            return f"Both chunking and direct processing failed: {str(e)}"
...
async def speech_to_text_from_audio(audio_data: bytes) -> str:
    ...
    try:
        ...
        if len(audio_data) > DIRECT_SUBMIT_LIMIT:
            ...
            return await _process_audio_in_chunks(audio_data, stt_tool)
